...
const CardItem = ({
    ...
    const { cartItems, productCount } = useSelector((state) => state.cartReducer);
    const itemInCart = cartItems.find((item) => item.id === id);
    const [countOfProduct, setCountOfProduct] = useState(quantity);

    const handleAddToCart = () => {
        if (!itemInCart) {
            dispatch(actions.addToCart({ id, title, price, image, weight, type, quantity: 1 }));
        }
    };

    console.log(cartItems, 'fdfd');
